...
class MessageDisplayMedia extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (_, constraints) => SizedBox(
        width: max(200, min(constraints.maxWidth, contentSize?.width.toDouble() ?? 300)),
        child: haveText
            ? MessageDisplayText(
                client: client,
                message: message,
  ...
  Widget _buildImage(BorderRadius border) => content; //ClipRRect(borderRadius: border, child: content);
}
