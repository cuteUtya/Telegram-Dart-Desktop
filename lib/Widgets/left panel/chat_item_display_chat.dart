...
class _ChatItemDisplayState extends State<ChatItemDisplay> {
  bool pinned(Chat chat) =>
      chat.positions?.firstWhere((element) => compareChatlists(element.list!, widget.chatList)).isPinned ?? false;
  ...
  @override
  void initState() {
    _selectedSub = UIEvents.selectedChat().listen((event) {
      if (widget.chatId == event && !selected) {
        setState(() => selected = true);
      } else if (widget.chatId != event && selected) {
  ...
  Widget build(BuildContext context) {
    ...
    var supergroup =
        chat.type is ChatTypeSupergroup ? widget.client.getSupergroup((chat.type as ChatTypeSupergroup).supergroupId!) : null;
    return ChatItemBase(
      selected: selected,
        ...
        children: [
          Expanded(
            child: isSavedMessages
                ...
                : StreamBuilder(
                    stream: isSavedMessages ? null : widget.client.senderName(MessageSenderChat(chatId: widget.chatId)),
          ...
          StreamBuilder(
            ...
            builder: (context, data) {
              var value = false;
              if (data.hasData) {
                value = (widget.client.getChat(widget.chatId).lastMessage?.id ?? 0) <= (data.data as int);
              }
